...
class FocalixedLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        dice = self.dice_loss(logits, targets)
        tv = self.tv_loss(logits, targets)
        return focal + dice + self.alpha * tv
...
if args.config:
    ...
    if not config_path.is_absolute():
        cwd_config_path = Path.cwd() / config_path
        config_path = cwd_config_path if cwd_config_path.exists() else PROJECT_ROOT / config_path
...
logger = SimpleLogger(str(LOGS_DIR), run_name=config.run_name)
...
torch.save(model, os.path.join(metric_manager.logger.saved_model_path, f"unet_epoch_{config.num_epochs}.pth"))
